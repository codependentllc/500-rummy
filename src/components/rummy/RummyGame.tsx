...
export function RummyGame() {
  ...
  const startGame = (scores: number[] = []) => {
    ...
    setGame(createGame(seats.length, seats, scores, setup.difficulty));
    setScreen("game");
  };
  ...
  if (screen === "cardBack") return <CardBackScreen selectedId={setup.cardBackId} difficulty={setup.difficulty} onChange={(cardBackId) => setSetup((current) => ({ ...current, cardBackId }))} onDifficultyChange={(difficulty) => setSetup((current) => ({ ...current, difficulty }))} onBack={() => setScreen("tableTheme")} onStart={() => startGame()} />;
  if (!game) return null;
  return <GameScreen game={game} tableThemeClass={tableTheme.className} cardBackClass={cardBack.className} setGame={(update) => setGame((current) => current ? update(current) : current)} onNewGame={resetGame} onNextHand={() => startGame(game.players.map((player) => player.score))} />;
}
