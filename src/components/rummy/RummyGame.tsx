...
import type { GameState, ScreenState } from "@/types/rummy";
...
export function RummyGame() {
  ...
  const startGame = (scores: number[] = []) => {
    setGame(createGame(playerCount, avatar, displayName, scores));
    setScreen("game");
  };
  ...
  if (screen === "setup") return <SetupScreen playerName={displayName} playerCount={playerCount} onCountChange={setPlayerCount} onStart={() => startGame()} />;
  if (!game) return null;
